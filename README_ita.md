...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Fase-1%2C%203%20%26%204%20di%206%20complete-367BF5.svg" alt="Fasi 1, 3 e 4 di 6 complete">
</p>

> **Stato: v0.1.1, funzionale - Fasi 1, 3 e 4 di 6 complete (conoscenza
> recuperabile, orchestratore di strumenti, escalation), Fase 5 avviata
> (una vera CLI locale).** La Fase 0 ha
> definito la politica reale dei livelli di rischio
> (`policy/risk_levels.py`), una lista bianca fissa di strumenti
> (`policy/tool_matrix.py`), i cinque contratti minimi reali che ogni
> futura chiamata a uno strumento dovrà validare
...
> né rivelare un segreto. La Fase 3 collega 9 dei strumenti OBSERVE
> dichiarati a un handler reale (`orchestrator/dispatch.py`):
> `service.status`, `storage.usage`, `network.port_status`,
> `network.connectivity`, `system.temperature`, `manifest.read`, `logs.read`, `process.list` e `update.pending` -
> ciascuno risolve solo un nome simbolico in whitelist
> (`orchestrator/allowlist.py`), mai un percorso/host/porta/URL grezzo
> che un documento recuperato potrebbe fornire. La Fase 1 aggiunge un
> decimo strumento, `knowledge.search` (`knowledge/index.py`) - un
> indice TF-IDF locale e reale su una radice in whitelist di
> documentazione, manifesti e contratti approvati, portato dal motore
> di recupero già testato di HYDRA-UMC-DOCS-QA. La Fase 4 aggiunge il
> vero assemblaggio di `EvidenceBundle` e la costruzione di
> `MaintenanceProposal` (`escalation/`) - fondati su output di strumenti
> già osservati, mai una diagnosi inventata: poiché non esiste ancora un
> motore di inferenza, `propose_maintenance()` si limita a validare e
> fondare un contenuto fornito da un operatore umano, non lo genera mai.
> La prima vera porzione della Fase 5 - `tools list`/`tools call` e
> `escalation evidence`/`escalation propose` - dà a ogni pezzo sopra una
> vera riga di comando, mai una nuova fonte di autorità: chiama solo lo
> stesso codice già validato dalla policy che ogni test già esercita.
> Non esiste ancora un motore di inferenza, né un'integrazione con
...
**Controllo di onestà - cosa funziona davvero oggi:** la policy dei livelli di rischio (`policy/risk_levels.py`), l'elenco fisso di strumenti consentiti (`policy/tool_matrix.py`), i cinque validatori di contratto (`contracts.py` + `contracts/*.schema.json`), il confine di difesa dalle injection (`knowledge/trust.py`, `knowledge/redaction.py`), l'indice di conoscenza TF-IDF locale (`knowledge/index.py`, Fase 1), tutti i 10 strumenti OBSERVE dichiarati con un handler reale (`orchestrator/dispatch.py` + `orchestrator/allowlist.py`: `service.status`, `storage.usage`, `network.port_status`, `network.connectivity`, `system.temperature`, `manifest.read`, `logs.read`, `process.list`, `update.pending`, `knowledge.search`), la costruzione reale di `EvidenceBundle`/`MaintenanceProposal` (`escalation/evidence.py`, `escalation/proposal.py`, Fase 4), e una vera CLI su tutto quanto sopra (`tools list`/`tools call`/`escalation evidence`/`escalation propose`, prima porzione della Fase 5) sono testati (164 test più 30 subtest superati tra `tests/unit/` e `tests/adversarial/`) - le Fasi 1, 3 e 4 sono complete, la Fase 5 è avviata. `update.pending` richiede la dipendenza opzionale `hydra-umc-updater` (extra `update-check`) per verificare GitHub davvero, e degrada onestamente (`available: false`) senza di essa. Non esiste ancora alcun motore di inferenza, né alcuna integrazione con HYDRA-UMC-SERVER in nessuna parte di questo repository, e nulla in questo codice può ancora agire su un `MaintenanceProposal` - la Fase 2, e il resto della Fase 5 (l'API integrata con HYDRA-UMC-SERVER, e la voce), nella Roadmap più sotto restano interamente aspirazionali, senza alcun codice dietro. Vedi `CHANGELOG.md` per ciò che è stato consegnato esattamente finora.
...
Questa consegna porta otto pezzi reali e utili in modo indipendente:
...
2. **Matrice degli strumenti** (`policy/tool_matrix.py`) - una lista
   bianca fissa di dieci nomi reali di strumenti di livello `OBSERVE`. Un
   nome di strumento assente da questo dizionario non può mai essere
   chiamato, punto - tutti i 10 sono già collegati a un handler reale
   (vedi i punti 5 e 6 più sotto).
...
5. **Orchestratore di strumenti** (`orchestrator/dispatch.py` +
   `orchestrator/allowlist.py`, Fase 3) - `dispatch_tool_request()`
   esegue davvero un `ToolRequest` già validato e restituisce un
   `ToolResult` reale: `service.status` (`systemctl is-active` reale),
   `storage.usage` (`shutil.disk_usage`), `network.port_status` (una
   vera sonda socket), `network.connectivity` (una vera richiesta HTTP
   GET che riporta il codice di stato reale dell'endpoint - una verifica
   genuinamente diversa da `network.port_status`, la stessa distinzione
   reale che il proprio `inventory.py` di HYDRA-UMC-OPS-AGENT traccia tra
   una semplice connessione TCP e un vero GET di salute),
   `system.temperature` (il vero percorso sysfs della zona termica
   Linux), `manifest.read` (una vera lettura di
   `hydra-umc.project.json`), `logs.read` (una coda limitata e redatta
   di un file di log in whitelist), `process.list` (una vera
   corrispondenza di sottostringa in whitelist contro
   `/proc/<pid>/cmdline`), e `update.pending` (riutilizza la vera
   scoperta GitHub e il confronto di versioni di HYDRA-UMC-UPDATER,
   dipendenza opzionale). Nessuno di questi accetta direttamente un
   percorso/host/porta/URL/nome unità/pattern grezzo - solo un nome
   simbolico breve risolto tramite un `OrchestratorConfig` fisso, così
   che il testo di un documento avvelenato non possa mai nominare un
   bersaglio reale arbitrario, solo un nome già in whitelist.
6. **Indice di conoscenza recuperabile** (`knowledge/index.py`, Fase 1)
   - un vero motore di ricerca TF-IDF locale
   (tokenize/build_index/search), portato senza modifiche dal già
   testato `index.py` di HYDRA-UMC-DOCS-QA, più una nuova ingestione
   limitata per fonti Markdown (frammenti per intestazione) e JSON (un
   intero file per frammento - un manifesto o uno schema di contratto
   non viene mai utilmente diviso a metà oggetto). Collegato come
   decimo strumento, `knowledge.search`, tramite il nuovo
   `resolve_knowledge_source()` di `orchestrator/allowlist.py`: chi
   chiama può nominare solo una fonte simbolica già in whitelist, mai un
   percorso grezzo, e ogni corrispondenza reale restituita resta
   avvolta come `UntrustedText` prima di lasciare l'handler. La
   scansione stessa è limitata (numero di file, dimensione per file)
   anche per una radice legittimamente in whitelist - mai alla cieca,
   mai tutto il disco.
7. **Escalation: prove e proposte** (`escalation/`, Fase 4) -
   `evidence.assemble_evidence_bundle()` trasforma un lotto reale di
   `ToolResult` già eseguiti e già validati in un `EvidenceBundle` reale,
   seguendo lo stesso schema del `incident.py` di HYDRA-UMC-OPS-AGENT:
   derivazione pura da dati già osservati, mai una chiamata IA.
   `proposal.propose_maintenance()` è un costruttore/validatore reale per
   `MaintenanceProposal` - deliberatamente NON un generatore, poiché non
   esiste ancora un motore di inferenza e il principio non negoziabile di
   questo progetto esclude di inventare una diagnosi. Oggi solo un
   operatore umano fornisce `diagnosis`/`steps`/`rollback`; l'unica vera
   salvaguardia che questo modulo aggiunge è che ogni voce di
   `citedEvidence` deve corrispondere letteralmente al campo `evidence`
   di un risultato di strumento già osservato, altrimenti la proposta
   viene rifiutata del tutto. `confirmationRequired` non è mai
   configurabile da chi chiama - è sempre esattamente ciò che la policy
   di `policy/risk_levels.py` già dice per il rischio dichiarato della
   proposta. Nulla in questo codice può ancora agire su un
   `MaintenanceProposal`.
8. **Superficie CLI** (`cli.py`, prima vera porzione della Fase 5) -
   `tools list`/`tools call` e `escalation evidence`/`escalation
   propose` danno per la prima volta a ogni pezzo sopra una vera riga di
   comando (finora ognuno era raggiungibile solo da un test). Non
   aggiunge autorità propria: `tools call` costruisce un `ToolRequest`
   con lo stesso `build_tool_request_from_model_output()` che ogni test
   già chiama e lo esegue con lo stesso `dispatch_tool_request()` -
   raggiunge sempre e solo uno strumento di livello OBSERVE, poiché
   nulla al di sopra di OBSERVE è implementato in questo codice.
   `--config` carica un vero `OrchestratorConfig` da un file JSON (o
   ricade su `default_config()`); resta impossibile nominare un
   percorso/host/porta grezzo direttamente sulla riga di comando.
...
- **Il recupero di conoscenza è limitato e in whitelist, mai alla
  cieca.** `knowledge.search` indicizza solo una radice già in whitelist
  (`resolve_knowledge_source()` di `orchestrator/allowlist.py`), e la
  scansione stessa è limitata da un vero limite di numero di file e di
  dimensione per file - "mai addestrato alla cieca su tutto il disco"
  vale anche per una radice che un operatore ha già deciso essere
  sicura.
- **Questa consegna dichiara, recupera e valida soltanto - non agisce
  ancora.** Non esiste ancora un motore di inferenza, né una vera
  esecuzione di strumenti oltre OBSERVE, né un'integrazione con
  HYDRA-UMC-SERVER in nessuna parte di questo repository.
...
│   │   └── tool_matrix.py    # TOOL_MATRIX: la lista bianca fissa e reale di strumenti (10 nomi OBSERVE)
│   ├── knowledge/
...
│   │   └── index.py          # Fase 1 - vera ricerca TF-IDF + ingestione limitata Markdown/JSON, portata da HYDRA-UMC-DOCS-QA
│   ├── orchestrator/          # Fase 3 + Fase 1 - tutti i 10 gestori reali di strumenti OBSERVE, completo
│   │   ├── allowlist.py      # OrchestratorConfig: whitelist di nomi simbolici che un gestore risolve, mai un percorso/host/porta grezzo
│   │   └── dispatch.py       # dispatch_tool_request(): esegue davvero un ToolRequest validato, restituisce un vero ToolResult
│   ├── escalation/            # Fase 4 - vero assemblaggio di EvidenceBundle + costruzione di MaintenanceProposal, completo
│   │   ├── evidence.py       # assemble_evidence_bundle(): veri ToolResult -> un EvidenceBundle reale e valido secondo il contratto
│   │   └── proposal.py       # propose_maintenance(): costruttore/validatore reale per MaintenanceProposal, mai un generatore
...
Questa versione porta la Fase 0, la Fase 1, la Fase 3 e la Fase 4. Ciò che resta, in ordine di fase:

- **Fase 1 - Conoscenza recuperabile (completa).** Un indice locale e
  versionato di documentazione, manifesti, contratti e runbook
  approvati - mai addestrato alla cieca su tutto il disco.
  `knowledge/index.py` porta senza modifiche la ricerca TF-IDF già
  testata di HYDRA-UMC-DOCS-QA e aggiunge un'ingestione limitata e in
  whitelist per fonti Markdown e JSON, collegata come decimo strumento
  di livello OBSERVE, `knowledge.search`.
- **Fase 2 - Motore di inferenza locale.** Un piccolo LLM compatibile
  ...
  la licenza.
- **Fase 3 - Orchestratore di strumenti (completa: 9 su 9 dichiarati).** Codice
  deterministico che collega tutti i gestori reali di strumenti di
  livello `OBSERVE` a `TOOL_MATRIX`, con la politica applicata a ogni
  chiamata. `service.status`, `storage.usage`, `network.port_status`,
  `network.connectivity`, `system.temperature`, `manifest.read`, `logs.read`, `process.list` e `update.pending` sono
  già reali (`orchestrator/dispatch.py`).
- **Fase 4 - Proposte e prove (completa).** Costruzione reale di
  `MaintenanceProposal` ed `EvidenceBundle` (`escalation/`), con
  escalation verso il futuro ruolo di Developer Node di
  HYDRA-UMC-DEV-SERVER. `evidence.assemble_evidence_bundle()` deriva un
  pacchetto reale da `ToolResult` già osservati;
  `proposal.propose_maintenance()` valida e fonda una proposta fornita
  da un umano contro prove reali - non la genera mai, poiché non esiste
  ancora un motore di inferenza. Nulla in questo codice può ancora agire
  su un `MaintenanceProposal`.
- **Fase 5 - Interfaccia utente (avviata: la porzione CLI).** Un'API
  locale integrata in HYDRA-UMC-SERVER/Studio, poi una CLI, poi la voce -
  senza mai aggirare i confini di politica e conferma già stabiliti dalla
  Fase 0. La porzione CLI è reale da subito: `tools list`/`tools call` e
  `escalation evidence`/`escalation propose` (`cli.py`) raggiungono ogni
  pezzo reale sopra attraverso lo stesso codice già testato e già forzato
  dalla policy, deliberatamente limitato a ciò che vive in questo stesso
  repository. L'API integrata con HYDRA-UMC-SERVER e la voce restano
  lavoro separato, tra repository, per una consegna futura.

La Fase 2, e il resto della Fase 5, non esistono ancora in questo repository - vedi
[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) per ciò che ogni fase
restante include ed esclude esplicitamente, e
